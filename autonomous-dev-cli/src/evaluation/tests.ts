...
async function determineTestCommands(repoPath: string, packages: string[]): Promise<TestCommand[]> {
 ...
 */
function parseNodeTestRunnerOutput(output: string): { total: number; passed: number; failed: number } | null {
 ...
 *   1..3
 */
function parseTapOutput(output: string): { total: number; passed: number; failed: number } {
