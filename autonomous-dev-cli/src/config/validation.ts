...
export function validateConfigPath(
  ...
  if (allowedExtensions.length > 0) {
    const hasValidExtension = allowedExtensions.some(ext =>
      absolutePath.toLowerCase().endsWith(ext.toLowerCase())
    );
 ...
 */
export function validateWorkDirectory(
  inputPath: string,
  options: PathValidationOptions = {}
): ValidationResult {
  const {
 ...
 */
export function validateIssueLabel(label: string): ValidationResult {
 ...
 */
export function validateExcludePaths(paths: string[]): ValidationResult {
 ...
 */
export function sanitizeForDisplay(input: string, maxLength: number = 100): string {
