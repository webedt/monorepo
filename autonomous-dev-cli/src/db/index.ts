...
export interface PoolConfig {
  ...
  reconnectOnError?: boolean; // Enable automatic reconnection (default: true)
  reconnectRetries?: number; // Number of reconnection attempts (default: 3)
  reconnectBaseDelayMs?: number; // Base delay for exponential backoff (default: 1000)
  healthCheckIntervalMs?: number; // Interval for health checks (default: 30000)
}
 ...
 */
export async function initDatabase(dbUrl: string, config: PoolConfig = {}): Promise<void> {
  if (pool) {
    return;
  }
 ...
 */
function startHealthCheckInterval(intervalMs: number): void {
 ...
 */
export async function performHealthCheck(): Promise<{ healthy: boolean; latencyMs: number; error?: string }> {
  ...
  try {
    ...
    if (poolConfig.reconnectOnError !== false && isNetworkError(error as Error)) {
