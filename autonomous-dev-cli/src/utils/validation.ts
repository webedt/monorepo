...
import { existsSync, accessSync, constants, realpathSync } from 'fs';
...
export const GitHubOwnerSchema = z.string()
  ...
  .regex(
    ...
    'Owner must be alphanumeric with hyphens, cannot start or end with hyphen'
  );
...
export const GitHubRepoNameSchema = z.string()
  ...
  .refine(
    ...
    'Repository name should not end with .git'
  );
...
export const EnvVarNameSchema = z.string()
  .regex(
    /^[A-Z_][A-Z0-9_]*$/,
    'Environment variable name must be uppercase letters, numbers, and underscores'
  );
...
export function validatePath(
  inputPath: string,
  allowedBase?: string
): ValidationResult & { sanitizedPath?: string } {
  ...
  if (containsPathTraversal(inputPath)) {
    return {
      ...
      error: new ValidationError(
        ...
        `Path contains potentially dangerous characters or traversal sequences: "${inputPath}"`,
        {
          field: 'path',
          value: inputPath,
  ...
  if (allowedBase) {
    ...
    if (relativePath.startsWith('..') || isAbsolute(relativePath)) {
      return {
        ...
        error: new ValidationError(
          ErrorCode.VALIDATION_INVALID_FORMAT,
          `Path "${inputPath}" resolves outside the allowed directory: "${allowedBase}"`,
          {
            field: 'path',
            value: inputPath,
 ...
 * @returns ValidationResult with sanitized path or error
 */
export function validateWorkDir(workDir: string): ValidationResult & { sanitizedPath?: string } {
  ...
  if (!isAllowed) {
    ...
    if (!isWithinCwd) {
      return {
        ...
        error: new ValidationError(
          ErrorCode.VALIDATION_INVALID_FORMAT,
          `Working directory "${workDir}" is outside allowed locations`,
          {
            field: 'workDir',
            value: workDir,
            constraints: [
 ...
 */
const ENV_VAR_VALIDATORS: Record<string, {
    ...
    validate: (value) => {
      if (!value || value.length < 10) {
        return {
          ...
          error: new ValidationError(
            ErrorCode.VALIDATION_INVALID_FORMAT,
            'CLAUDE_ACCESS_TOKEN appears to be too short or invalid',
            {
    ...
    validate: (value) => {
      ...
      if (!result.success) {
        return {
          ...
          error: new ValidationError(
            ErrorCode.VALIDATION_INVALID_FORMAT,
            `Invalid REPO_OWNER: ${result.error.errors[0].message}`,
            { field: 'REPO_OWNER', value }
          ),
        };
      }
      return { valid: true };
    ...
    validate: (value) => {
      ...
      if (!result.success) {
        return {
          ...
          error: new ValidationError(
            ErrorCode.VALIDATION_INVALID_FORMAT,
            `Invalid REPO_NAME: ${result.error.errors[0].message}`,
            { field: 'REPO_NAME', value }
          ),
        };
      }
      return { valid: true };
...
export function validateEnvironmentVariables(
  ...
  isValid: boolean;
  errors: { envVar: string; error: ValidationError }[];
  warnings: { envVar: string; message: string }[];
} {
  const errors: { envVar: string; error: ValidationError }[] = [];
  const warnings: { envVar: string; message: string }[] = [];
 ...
 */
export function displayEnvValidationErrors(
  result: ReturnType<typeof validateEnvironmentVariables>
): void {
