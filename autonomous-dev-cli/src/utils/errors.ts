...
function getWorkspaceSeverity(code: ErrorCode): ErrorSeverity {
  switch (code) {
...
function isWorkspaceRetryable(code: ErrorCode): boolean {
  return code === ErrorCode.WORKSPACE_CREATE_FAILED;
}

function getWorkspaceRecoveryActions(code: ErrorCode, path?: string): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
...
function getBuildSeverity(code: ErrorCode): ErrorSeverity {
  switch (code) {
...
function isBuildRetryable(code: ErrorCode): boolean {
  return code === ErrorCode.BUILD_TIMEOUT;
}

function getBuildRecoveryActions(code: ErrorCode, buildOutput?: string): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
...
export class TestError extends StructuredError {
  ...
  public readonly testsRun?: number;
...
function getTestSeverity(code: ErrorCode): ErrorSeverity {
  switch (code) {
...
function isTestRetryable(code: ErrorCode): boolean {
  return code === ErrorCode.TEST_TIMEOUT || code === ErrorCode.TEST_ENVIRONMENT_ERROR;
}

function getTestRecoveryActions(code: ErrorCode, testOutput?: string): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
 ...
 */
export function formatUserFriendlyError(error: StructuredError): string {
  const lines: string[] = [];
 ...
 */
export function isCriticalError(error: StructuredError): boolean {
