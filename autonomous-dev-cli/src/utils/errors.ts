...
function getDatabaseSeverity(code: ErrorCode): ErrorSeverity {
  switch (code) {
...
function isDatabaseRetryable(code: ErrorCode): boolean {
  const retryableCodes = [
    ErrorCode.DB_CONNECTION_FAILED,
    ErrorCode.DB_QUERY_TIMEOUT,
    ErrorCode.DB_POOL_EXHAUSTED,
...
function getDatabaseRecoveryActions(code: ErrorCode): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
...
function getValidationRecoveryActions(code: ErrorCode, field?: string): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
...
function getConflictSeverity(code: ErrorCode): ErrorSeverity {
  switch (code) {
...
function isConflictRetryable(code: ErrorCode): boolean {
  const retryableCodes = [
    ErrorCode.CONFLICT_MERGE_FAILED,
    ErrorCode.CONFLICT_CONCURRENT_EDIT,
  ];
...
function getConflictRecoveryActions(code: ErrorCode): RecoveryAction[] {
  const actions: RecoveryAction[] = [];
 ...
 */
export function createErrorLoggingWrapper<TArgs extends unknown[], TResult>(
  fn: (...args: TArgs) => Promise<TResult>,
  logger: ErrorLogger,
  options: WithErrorLoggingOptions
): (...args: TArgs) => Promise<TResult> {
  return (...args: TArgs) => withErrorLogging(() => fn(...args), logger, options);
}
