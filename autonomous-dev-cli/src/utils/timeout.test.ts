...
describe('timeout utilities', () => {
  ...
  describe('withTimeout', () => {
    ...
    it('should throw TimeoutError when operation times out', async () => {
      await assert.rejects(
        ...
        (error: Error) => {
          ...
          assert.strictEqual((error as TimeoutError).operationName, 'slowOperation');
          return true;
        }
      );
    });
    ...
    it('should include context in TimeoutError', async () => {
      try {
        ...
        assert.ok(error instanceof TimeoutError);
        assert.strictEqual((error as TimeoutError).context.customKey, 'customValue');
      }
    });
  });
  ...
  describe('withTimeoutDetailed', () => {
    it('should return result with duration', async () => {
      const result = await withTimeoutDetailed(
        async () => 'success',
        { timeoutMs: 1000, operationName: 'test' }
      );
  ...
  describe('withGitHubTimeout', () => {
    ...
    it('should complete before timeout', async () => {
      const result = await withGitHubTimeout(
        async () => 'success',
        'test'
      );
      assert.strictEqual(result, 'success');
  ...
  describe('withTimeoutAll', () => {
    it('should return results for all completed operations', async () => {
      ...
      if (results[0].success) assert.strictEqual(results[0].result, 'result1');
      assert.strictEqual(results[1].success, true);
      if (results[1].success) assert.strictEqual(results[1].result, 'result2');
    });
