...
import { createChecksManager } from './checks.js';
import { logger } from '../utils/logger.js';
import { retryWithBackoff, RATE_LIMIT_RETRY_CONFIG, extractRetryAfterMs, } from '../utils/retry.js';
...
(function (OperationPriority) {
    ...
    OperationPriority[OperationPriority["CRITICAL"] = 100] = "CRITICAL";
    /** High priority operations (e.g., merges, PR creation) */
    OperationPriority[OperationPriority["HIGH"] = 75] = "HIGH";
    /** Normal priority operations (default) */
    OperationPriority[OperationPriority["NORMAL"] = 50] = "NORMAL";
    /** Low priority operations (e.g., listing, fetching) */
    OperationPriority[OperationPriority["LOW"] = 25] = "LOW";
    /** Background operations that can wait */
    OperationPriority[OperationPriority["BACKGROUND"] = 0] = "BACKGROUND";
...
export class GitHubManager {
    ...
    config;
    rateLimiter;
    log = logger.child('GitHubManager');
    operationQueue = [];
    ...
    getHealth() {
        ...
        else if (clientHealth.status === 'degraded' ||
            this.isThrottling() ||
    ...
    async execute(operation, options = {}) {
        const { description = 'GitHub API operation', priority = OperationPriority.NORMAL, resource = 'core', skipQueue = false, maxRetries = RATE_LIMIT_RETRY_CONFIG.maxRetries, } = options;
        ...
        try {
            this.activeRequests++;
            const result = await retryWithBackoff(async (context) => {
     ...
     */
    enqueueOperation(operation, description, priority, resource) {
        ...
        if (this.operationQueue.length >= maxQueueSize) {
            ...
            throw new GitHubError(ErrorCode.GITHUB_RATE_LIMITED, `Operation queue full (${this.operationQueue.length}/${maxQueueSize}). ${description}`, {
                statusCode: 429,
                context: {
                    queueSize: this.operationQueue.length,
                    description,
        ...
        return new Promise((resolve, reject) => {
            ...
            queuedOp.timeout = setTimeout(() => {
                ...
                if (index !== -1) {
                    ...
                    reject(new GitHubError(ErrorCode.GITHUB_RATE_LIMITED, `Operation timed out waiting in queue (waited ${Math.round(waitTime / 1000)}s): ${description}`, {
                        statusCode: 429,
    ...
    recordResponseTime(responseTimeMs) {
        ...
        this.stats.averageResponseTimeMs = Math.round(this.totalResponseTimeMs / this.stats.successfulRequests);
    }
    /**
    ...
    clearQueue() {
        ...
        for (const operation of this.operationQueue) {
            ...
            operation.reject(new GitHubError(ErrorCode.GITHUB_SERVICE_DEGRADED, 'Operation queue cleared', { statusCode: 503, context: { operationId: operation.id } }));
        }
        this.operationQueue = [];
        this.stopQueueProcessor();
 ...
 */
export function createGitHubManager(config) {
