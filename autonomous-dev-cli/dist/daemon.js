...
import { cleanupOrphanedWorkspaces, processDeferredCleanup, getCleanupStatus, } from './utils/cleanup.js';
...
export class Daemon {
     ...
     */
    async cleanupOrphanedWorkspacesOnStartup() {
        logger.info('Checking for orphaned workspaces from previous runs...');
        try {
            const result = await cleanupOrphanedWorkspaces(this.config.execution.workDir, {
                maxRetries: 3,
                retryDelayMs: 500,
                timeoutMs: 30000,
            ...
            logger.warn(`Orphaned workspace cleanup failed: ${error.message}`);
     ...
     */
    async processDeferredCleanupItems() {
        const cleanupStatus = getCleanupStatus();
        ...
        try {
            ...
            logger.warn(`Deferred cleanup processing failed: ${error.message}`);
        }
    }
    async shutdown() {
