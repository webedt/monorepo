...
Start by exploring the codebase, then implement the required changes.`;
    ...
    describe('ExecutionError', () => {
        it('should create error with clone failed code', () => {
            const error = new ExecutionError(ErrorCode.EXEC_CLONE_FAILED, 'Failed to clone repository', {
        ...
        it('should create error with commit failed code', () => {
            const error = new ExecutionError(ErrorCode.EXEC_COMMIT_FAILED, 'Failed to commit changes', {
                issueNumber: 42,
                branchName: 'feature/test',
            });
        ...
        it('should create error with push failed code', () => {
            const error = new ExecutionError(ErrorCode.EXEC_PUSH_FAILED, 'Failed to push changes', {
                issueNumber: 42,
                branchName: 'feature/test',
        ...
        it('should include recovery actions', () => {
            const error = new ExecutionError(ErrorCode.EXEC_CLONE_FAILED, 'Network error during clone');
        ...
        it('should preserve cause chain', () => {
            ...
            const error = new ExecutionError(ErrorCode.EXEC_PUSH_FAILED, 'Push failed', { cause });
    ...
    describe('retry configuration', () => {
        ...
        it('should identify retryable network errors', () => {
            ...
            retryableMessages.forEach((msg) => {
                const isRetryable = msg.includes('network') ||
...
describe('Worker integration scenarios', () => {
    ...
    describe('failure scenarios', () => {
        it('should handle clone failure', () => {
            const error = new ExecutionError(ErrorCode.EXEC_CLONE_FAILED, 'Failed to clone: network error');
            assert.strictEqual(error.code, ErrorCode.EXEC_CLONE_FAILED);
        });
        ...
        it('should handle commit failure', () => {
            const error = new ExecutionError(ErrorCode.EXEC_COMMIT_FAILED, 'Commit failed: pre-commit hook error');
            assert.strictEqual(error.code, ErrorCode.EXEC_COMMIT_FAILED);
        });
        it('should handle push failure', () => {
            const error = new ExecutionError(ErrorCode.EXEC_PUSH_FAILED, 'Push failed: permission denied');
            assert.strictEqual(error.code, ErrorCode.EXEC_PUSH_FAILED);
        });
        it('should handle timeout', () => {
            const error = new ExecutionError(ErrorCode.EXEC_TIMEOUT, 'Task timed out after 30 minutes');
