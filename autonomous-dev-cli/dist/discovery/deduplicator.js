...
export class TaskDeduplicator {
     ...
     */
    async deduplicateTasks(tasks, existingIssues) {
    ...
    calculateTaskSimilarity(task1, task2) {
        ...
        const { overlap: pathOverlap, overlappingPaths } = this.calculatePathOverlap(task1.affectedPaths, task2.affectedPaths);
        const criticalFilesInCommon = this.findCriticalFilesInCommon(task1.affectedPaths, task2.affectedPaths);
        ...
        const score = (titleSimilarity * 0.3 +
            pathOverlap * 0.5 +
     ...
     */
    calculateTaskIssueSimilarity(task, issue) {
        ...
        const { overlap: pathOverlap, overlappingPaths } = this.calculatePathOverlap(task.affectedPaths, issuePaths);
        const criticalFilesInCommon = this.findCriticalFilesInCommon(task.affectedPaths, issuePaths);
        const sharesCriticalFiles = criticalFilesInCommon.length > 0;
        const score = (titleSimilarity * 0.3 +
            pathOverlap * 0.5 +
            (sharesCriticalFiles ? 0.2 : 0));
     ...
     */
    predictConflict(task, existingIssues, issuePathMap) {
        const reasons = [];
        const conflictingIssues = [];
        const criticalFilesModified = [];
        let riskScore = 0;
     ...
     */
    processTask(task, existingIssues, issuePathMap, processedTasks) {
        const relatedIssues = [];
     ...
     */
    calculatePathOverlap(paths1, paths2) {
     ...
     */
    findCriticalFilesInCommon(paths1, paths2) {
 ...
 */
export function groupTasksByConflict(tasks) {
    const groups = [];
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        for (let j = i + 1; j < tasks.length; j++) {
            ...
            const sharesConflicts = conflicting1.some(c => task2.conflictPrediction.conflictingIssues.includes(c));
