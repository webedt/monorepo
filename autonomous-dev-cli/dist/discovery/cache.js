...
export class AnalysisCache {
     ...
     */
    getTrackedFiles(excludePaths) {
        const files = [];
        const excludeSet = new Set(excludePaths);
        const scanDir = (dirPath, depth = 0) => {
            if (depth > 10)
                return; // Limit recursion depth for performance
            try {
     ...
     */
    shouldExclude(relativePath, excludePaths) {
     ...
     */
    saveCache(analysis, excludePaths, analyzerConfig) {
        if (!this.enabled) {
            return;
        }
        try {
     ...
     */
    validateCache(excludePaths, analyzerConfig) {
        if (!this.enabled) {
        ...
        if (entry.analyzerConfig.maxDepth !== analyzerConfig.maxDepth ||
     ...
     */
    getCachedAnalysis(excludePaths, analyzerConfig) {
     ...
     */
    getIncrementalUpdate(changedFiles, fullAnalyze) {
        const entry = this.loadCache();
        if (!entry) {
            return null;
        }
 ...
 */
export function createAnalysisCache(repoPath, config) {
