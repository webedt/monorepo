...
import { TaskGenerator, discoverTasks, } from './generator.js';
  ...
  },
  {
    ...
    describe('task validation', () => {
        it('should filter out invalid tasks', () => {
            const tasks = [
                ...
                createMockTask({ priority: 'invalid' }), // Invalid priority
            ];
    ...
    describe('ClaudeError handling', () => {
        it('should create error for 401 status', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_AUTH_FAILED, 'Claude API authentication failed');
        ...
        it('should create error for 403 status', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_AUTH_FAILED, 'Claude API access denied');
            assert.strictEqual(error.code, ErrorCode.CLAUDE_AUTH_FAILED);
        });
        it('should create error for 429 status', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_RATE_LIMITED, 'Claude API rate limited');
            assert.strictEqual(error.code, ErrorCode.CLAUDE_RATE_LIMITED);
            assert.ok(error.isRetryable);
        });
        it('should create error for timeout', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_TIMEOUT, 'Claude API request timed out');
            assert.strictEqual(error.code, ErrorCode.CLAUDE_TIMEOUT);
            assert.ok(error.isRetryable);
        });
        it('should create error for server errors', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_API_ERROR, 'Claude API server error (500)');
            assert.strictEqual(error.code, ErrorCode.CLAUDE_API_ERROR);
        });
        it('should create error for invalid response', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_INVALID_RESPONSE, 'No JSON array found in Claude response');
        ...
        it('should include recovery actions for rate limit errors', () => {
            const error = new ClaudeError(ErrorCode.CLAUDE_RATE_LIMITED, 'Rate limited');
            assert.ok(error.recoveryActions.length > 0);
        });
    });
