...
import { ConflictResolver, createConflictResolver, } from './resolver.js';
// Create mock PR
...
function createMockBranchManager(overrides = {}) {
    return {
...
describe('ConflictResolver', () => {
    ...
    describe('constructor', () => {
        ...
        it('should accept different merge strategies', () => {
            const strategies = ['rebase', 'merge', 'manual'];
    ...
    describe('mergeSequentially', () => {
        ...
        it('should continue processing after a failure', async () => {
            ...
            const findPRForBranch = mock.fn(async () => {
                callCount++;
                if (callCount === 1)
                    return null; // First branch fails
                return createMockPR();
            });
            const prManager = createMockPRManager({ findPRForBranch });
            const resolver = new ConflictResolver({
        ...
        it('should process branches in order', async () => {
            const processOrder = [];
            const findPRForBranch = mock.fn(async (branchName) => {
