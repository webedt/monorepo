 ...
 */
import { CURRENT_CONFIG_VERSION, SUPPORTED_CONFIG_VERSIONS } from './schema.js';
...
migrations.set(1, (config) => {
    ...
    if (config.debug !== undefined) {
        warnings.push('The "debug" field is deprecated. Use "logging.level: debug" instead. ' +
    ...
    if (config.verbose !== undefined) {
        warnings.push('The "verbose" field is deprecated. Use "logging.level: debug" instead. ' +
            'Example: { "logging": { "level": "debug" } }');
        changes.push({
            type: 'deprecated',
            path: 'verbose',
...
export function isVersionSupported(version) {
    return SUPPORTED_CONFIG_VERSIONS.includes(version);
}
/**
 ...
 */
export function checkDeprecatedFields(config) {
    const warnings = [];
 ...
 */
export function formatMigrationSummary(result) {
    const lines = [];
    if (result.success) {
