import { z } from 'zod';
import { validateRepoOwner, validateRepoName, validateBranchName, validateWorkDirectory, validateIssueLabel, validateUrl, containsPathTraversal, sanitizePath, } from './validation.js';
/**
...
const safePathSchema = z.string()
    .refine((val) => !containsPathTraversal(val), 'Path contains invalid traversal sequences')
...
const urlSchema = z.string()
    .refine((val) => {
    if (!val)
        return true; // Allow empty strings (will be filtered out)
