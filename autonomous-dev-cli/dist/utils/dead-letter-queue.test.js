...
import { DeadLetterQueue, getDeadLetterQueue, resetDeadLetterQueue, } from './dead-letter-queue.js';
...
describe('DeadLetterQueue', () => {
    ...
    describe('constructor', () => {
        ...
        it('should load from disk when persistence is enabled', () => {
            ...
            const fs = require('fs');
            fs.writeFileSync(join(persistDir, 'dlq', 'dlq.json'), JSON.stringify([mockEntry]));
    ...
    describe('createEntryFromRetryContext', () => {
        it('should create entry from retry context', () => {
            ...
            const id = dlq.createEntryFromRetryContext('task-123', 'issue', 'owner/repo', retryHistory, {
                code: ErrorCode.EXEC_FAILED,
                message: 'Final error',
                severity: 'error',
                isRetryable: true,
            }, { workerId: 'worker-1' }, { issueNumber: 42, branchName: 'auto/42-feature' });
            const entry = dlq.getEntry(id);
            assert.ok(entry);
        ...
        it('should set canReprocess based on isRetryable', () => {
            const id = dlq.createEntryFromRetryContext('task-456', 'pr', 'owner/repo', [{ attemptNumber: 1, timestamp: new Date().toISOString(), errorMessage: 'Error', delayMs: 1000 }], {
...
function createMockEntryData(overrides = {}) {
    return {
        taskId: 'task-123',
        taskType: 'issue',
        lastAttemptAt: new Date().toISOString(),
...
//# sourceMappingURL=dead-letter-queue.test.js.map