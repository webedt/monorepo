...
import { ErrorCode, StructuredError, GitHubError, ClaudeError, ConfigError, ExecutionError, AnalyzerError, DEFAULT_RETRY_CONFIG, withRetry, wrapError, createGitHubErrorFromResponse, formatError, } from './errors.js';
...
describe('StructuredError', () => {
    ...
    describe('toJSON', () => {
        it('should serialize error to JSON', () => {
            ...
            assert.strictEqual(json.context.endpoint, '/api/test');
...
describe('GitHubError', () => {
    ...
    it('should include recovery actions for conflict errors', () => {
        ...
        assert.ok(error.recoveryActions.length > 0);
        assert.ok(error.recoveryActions.some((a) => a.description.toLowerCase().includes('rebase') || a.description.toLowerCase().includes('conflict')));
    });
});
...
describe('createGitHubErrorFromResponse', () => {
    ...
    it('should include endpoint in context', () => {
        const error = createGitHubErrorFromResponse({ status: 500, message: 'Error' }, '/repos/owner/repo');
        assert.strictEqual(error.context.endpoint, '/repos/owner/repo');
    });
    it('should include additional context', () => {
        const error = createGitHubErrorFromResponse({ status: 500, message: 'Error' }, '/api', { requestId: '123' });
