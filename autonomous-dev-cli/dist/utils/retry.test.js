...
import { retryWithBackoff, retryWithBackoffDetailed, calculateBackoffDelay, calculateProgressiveTimeout, createRetryContext, updateRetryContext, markContextFailed, classifyError, isErrorCodeRetryable, isHttpStatusRetryable, isNetworkErrorRetryable, extractHttpStatus, extractNetworkErrorCode, extractRetryAfterMs, createClaudeErrorFromResponse, isClaudeErrorRetryable, API_RETRY_CONFIG, NETWORK_RETRY_CONFIG, RATE_LIMIT_RETRY_CONFIG, CLAUDE_RETRY_CONFIG, DATABASE_RETRY_CONFIG, } from './retry.js';
...
describe('updateRetryContext', () => {
    it('should update context with attempt info', () => {
        ...
        const classification = { isRetryable: true, errorType: 'network' };
    ...
    it('should record attempt in history', () => {
        ...
        const classification = { isRetryable: true, errorType: 'structured' };
...
describe('isErrorCodeRetryable', () => {
    ...
    it('should return false for unknown codes', () => {
        assert.strictEqual(isErrorCodeRetryable('UNKNOWN_CODE'), false);
    });
});
...
describe('retryWithBackoff', () => {
    ...
    it('should call onRetry callback', async () => {
        const retryCalls = [];
        await retryWithBackoff(async (context) => {
    ...
    it('should respect abort signal', async () => {
        ...
        controller.abort();
        await assert.rejects(async () => {
            await retryWithBackoff(async () => 'success', {
...
describe('retryWithBackoffDetailed', () => {
    ...
    it('should track multiple attempts', async () => {
        let attempts = 0;
        const result = await retryWithBackoffDetailed(async () => {
            attempts++;
            if (attempts < 3) {
                const error = new Error('Error');
                error.status = 500;
