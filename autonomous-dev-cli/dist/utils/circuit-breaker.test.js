...
import { CircuitBreaker, getClaudeCircuitBreaker, getClaudeSDKCircuitBreaker, resetAllCircuitBreakers, getAllCircuitBreakerHealth, DEFAULT_CIRCUIT_BREAKER_CONFIG, } from './circuit-breaker.js';
...
describe('CircuitBreaker', () => {
    ...
    describe('executeWithRetry', () => {
        ...
        it('should throw circuit open error when circuit opens during retry', async () => {
            ...
            await assert.rejects(async () => {
                await cb.executeWithRetry(async () => {
                    const error = new Error('Always fails');
                    error.status = 500;
                    throw error;
                }, { maxRetries: 10 });
