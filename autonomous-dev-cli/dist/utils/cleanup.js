 ...
 */
const deferredCleanupQueue = new Set();
/**
 ...
 */
export function getDeferredCleanupQueueSize() {
    return deferredCleanupQueue.size;
}
/**
 * Get the deferred cleanup paths
 */
export function getDeferredCleanupPaths() {
 ...
 */
function fixPermissionsRecursive(path) {
    try {
 ...
 */
function incrementalCleanup(path) {
    if (!existsSync(path))
        return;
    const stats = statSync(path);
    if (!stats.isDirectory()) {
...
export async function cleanupWorkspace(path, config = {}, log) {
    ...
    const cleanupError = new StructuredError(ErrorCode.CLEANUP_FAILED, `Failed to cleanup workspace after ${retries} attempts: ${path}`, {
        severity: 'warning',
        context: {
            path,
            retries,
 ...
 */
function categorizeCleanupError(error) {
    if (!error)
        return 'unknown';
 ...
 */
export async function processDeferredCleanup(config = {}, log) {
 ...
 */
export async function cleanupOrphanedWorkspaces(workDir, config = {}, log) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                cleanupLog.warn(`Error processing ${entry}: ${error.message}`);
                failed++;
            }
        }
    }
    catch (error) {
        cleanupLog.error(`Failed to scan work directory: ${error.message}`);
    }
 ...
 */
function logDiskUsageStatus(log) {
    try {
