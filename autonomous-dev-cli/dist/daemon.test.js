...
import { Daemon, createDaemon } from './daemon.js';
...
describe('Daemon lifecycle states', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(testDir, 'autonomous-dev.config.json'), JSON.stringify(config));
    });
    afterEach(() => {
        process.chdir(originalCwd);
        if (existsSync(testDir)) {
...
describe('Error wrapping and context', () => {
    it('should properly create StructuredError', () => {
        const error = new StructuredError(ErrorCode.NOT_INITIALIZED, 'Daemon not properly initialized', {
            severity: 'critical',
            context: {
                operation: 'runCycle',
                component: 'Daemon',
    ...
    it('should handle error context chain', () => {
        ...
        const error = new StructuredError(ErrorCode.INTERNAL_ERROR, 'Wrapped error', { cause });
