...
const EXPECTED_TABLES = [
  {
    ...
    requiredColumns: [
      ...
      'preferred_provider', 'created_at', 'role', 'is_admin'
    ]
  },
  {
    name: 'sessions',
  ...
  },
  {
    ...
    requiredColumns: [
      'id', 'user_id', 'repository_owner', 'repository_name', 'base_branch',
      'working_branch', 'session_path', 'request_document', 'status',
      'current_cycle', 'max_parallel_tasks', 'provider', 'created_at'
    ]
  },
  {
    name: 'orchestrator_cycles',
    requiredColumns: [
      'id', 'job_id', 'cycle_number', 'phase', 'tasks_discovered',
      'tasks_launched', 'tasks_completed', 'tasks_failed', 'started_at'
    ]
  },
  {
...
async function createInitialSchema(pool: pg.Pool): Promise<void> {
  await pool.query(`
    ...
    -- Indexes for orchestrator tables
