...
usersCommand
  ...
  .action(async (options) => {
    try {
      ...
      // Build query with where clause BEFORE orderBy/limit
      const baseQuery = db
        .select({
          id: users.id,
          email: users.email,
      ...
      // Apply where clause first, then orderBy and limit
      const userList = options.role
        ? await baseQuery
            .where(eq(users.role, options.role))
