...
usersCommand
  ...
  .option('-r, --role <role>', 'User role (user, editor, developer, admin)', 'user')
  .action(async (email, password, options) => {
    try {
      ...
      if (!validRoles.includes(options.role)) {
        console.error(`Invalid role '${options.role}'. Valid roles: ${validRoles.join(', ')}`);
        process.exit(1);
      }
      ...
      const role = options.admin ? 'admin' : options.role;
...
usersCommand
  ...
  .action(async (userId, isAdmin) => {
    try {
      ...
      const updates: { isAdmin: boolean; role?: 'user' | 'editor' | 'developer' | 'admin' } = { isAdmin: adminStatus };
      if (adminStatus) {
        updates.role = 'admin';
      }
