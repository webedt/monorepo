...
webCommand
  ...
  .option('--check-events', 'Also check if session has a result event (default: true)')
  .option('--no-check-events', 'Only check session status, skip event check')
  .option('--json', 'Output as JSON')
  .action(async (sessionId, options, cmd) => {
    try {
      const parentOpts = cmd.parent?.opts() || {};
      ...
      const checkEvents = options.checkEvents !== false;
      const result = await client.isComplete(sessionId, checkEvents);
      ...
      handleCommandError(error, 'checking session completion', { json: options.json });
    }
  });
