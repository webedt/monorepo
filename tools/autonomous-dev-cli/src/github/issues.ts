...
export interface IssueManager {
  ...
  addCommentWithFallback(issueNumber: number, body: string): Promise<DegradedResult<Comment | null>>;
...
export function createIssueManager(client: GitHubClient): IssueManager {
  ...
  return {
      ...
      try {
        return await client.execute(
          async () => {
            ...
            logger.debug(`Added comment to issue #${issueNumber}`, { commentId: data.id });
    ...
    async addCommentWithFallback(issueNumber: number, body: string): Promise<DegradedResult<Comment | null>> {
      const result = await client.executeWithFallback(
        async () => {
          const { data } = await octokit.issues.createComment({
            owner,
    ...
    async listComments(issueNumber: number): Promise<Comment[]> {
      ...
      return client.getCachedOrFetch('comment-list' as CacheKeyType, cacheKey, async () => {
        try {
          return await client.execute(
            async () => {
    ...
    async getComment(commentId: number): Promise<Comment | null> {
      ...
      try {
        return await client.getCachedOrFetch('comment' as CacheKeyType, cacheKey, async () => {
          return await client.execute(
            async () => {
