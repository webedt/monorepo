...
/**
 * Configuration structure built by wizard
 */
...
export class ConfigWizard {
  ...
  private options: ConfigWizardOptions;
   ...
   */
  async run(): Promise<{ configPath: string; envPath?: string } | null> {
    try {
   ...
   */
  private displayStepHeader(stepNumber: number, title: string, description?: string): void {
  ...
  private async promptWithValidation(
    message: string,
    validator: (value: string) => LiveValidationResult,
   ...
   */
  private async stepRepository(): Promise<void> {
    this.displayStepHeader(
      1,
      'Repository Settings',
      'Configure the GitHub repository autonomous-dev will work with.'
    );

    // Repository owner
    const owner = await this.promptWithValidation(
  ...
  private async stepDiscovery(): Promise<void> {
    ...
    const tasksPerCycle = await this.promptNumber(
      'Tasks to discover per cycle',
      'tasksPerCycle',
      NUMERIC_RANGES.taskCount
    );

    const maxOpenIssues = await this.promptNumber(
      'Maximum open issues before pausing',
      'maxOpenIssues',
      NUMERIC_RANGES.maxOpenIssues
    );
   ...
   */
  private async stepExecution(): Promise<void> {
    this.displayStepHeader(
      3,
      'Execution Settings',
    ...
    const parallelWorkers = await this.promptNumber(
      'Parallel workers',
      'parallelWorkers',
      NUMERIC_RANGES.workerCount
    );

    const timeoutMinutes = await this.promptNumber(
      'Task timeout in minutes',
      'timeoutMinutes',
      NUMERIC_RANGES.timeoutMinutes
    );
   ...
   */
  private async stepEvaluation(): Promise<void> {
    this.displayStepHeader(
      4,
      'Evaluation Settings',
      'Configure quality checks before merging PRs.'
    );
    ...
    const checks = [];
    if (requireBuild) checks.push('build');
    if (requireTests) checks.push('tests');
    if (requireHealthCheck) checks.push('health');
    if (requireSmokeTests) checks.push('smoke');
   ...
   */
  private async stepMerge(): Promise<void> {
    this.displayStepHeader(
      5,
      'Merge Settings',
      'Configure how pull requests are merged.'
    );
   ...
   */
  private async stepCredentials(): Promise<void> {
    this.displayStepHeader(
      6,
      'Credentials Setup',
   ...
   */
  private async finalize(): Promise<{ configPath: string; envPath?: string }> {
    console.log();
  ...
  private displayConfigSummary(): void {
    ...
    const checks = [];
    if (c.evaluation?.requireBuild) checks.push('build');
    if (c.evaluation?.requireTests) checks.push('tests');
    if (c.evaluation?.requireHealthCheck) checks.push('health');
    if (c.evaluation?.requireSmokeTests) checks.push('smoke');
   ...
   */
  private buildFinalConfig(): WizardConfig {
    return {
   ...
   */
  private generateConfigWithComments(config: WizardConfig): string {
 ...
 */
export async function runConfigWizard(options: ConfigWizardOptions = {}): Promise<{ configPath: string; envPath?: string } | null> {
 ...
 */
export async function validateConfiguration(configPath?: string): Promise<ConfigValidationResult> {
  ...
  const repo = config.repo as { owner?: string; name?: string; baseBranch?: string } | undefined;
  if (!repo?.owner) {
    result.errors.push({
      field: 'repo.owner',
 ...
 */
export function displayValidationResults(result: ConfigValidationResult): void {
  console.log();
 ...
 */
export function generateExampleConfig(outputPath: string = './autonomous-dev.config.example.json'): void {
  const exampleConfig = {
    version: CURRENT_CONFIG_VERSION,
    repo: {
      owner: 'your-username',
