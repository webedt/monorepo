    ...
    if (workspacePath) {
      try {
        ...
        if (config.preview_url) {
          console.log('[PreviewUrlHelper] Using preview URL from .webedt file:', config.preview_url);
          return config.preview_url;
        }
      } catch (error) {
        // .webedt file doesn't exist or is invalid - this is expected for most repos
        if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
          console.warn('[PreviewUrlHelper] Failed to parse .webedt file, using default preview URL:', error);
        }
      }
    }
...
export async function readWebedtConfig(workspacePath: string): Promise<WebedtConfig | null> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      console.warn('[PreviewUrlHelper] Failed to read .webedt file:', error);
    }
    return null;
  }
}
