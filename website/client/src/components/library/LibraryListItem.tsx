...
/**
 * LibraryListItem - List view row component for library items.
 * Supports both detailed (standard list) and compact (dense list) variants.
 */
export default function LibraryListItem({
  ...
  const btnClasses = isCompact ? 'btn-xs' : 'btn-sm';
  const iconClasses = isCompact ? 'h-4 w-4' : 'h-5 w-5';
  const thumbnailClasses = isCompact ? 'w-10 h-10' : 'w-24 h-24';
  ...
  const renderFavoriteButton = () => (
    ...
    >
      <svg
        ...
        className={`${iconClasses} ${isFavorite ? 'text-warning fill-warning' : ''}`}
        viewBox="0 0 24 24"
  ...
  // Compact variant - dense list for power users
  if (isCompact) {
    return (
      <div
      ...
      >
        ...
        <img
          src={item.thumbnail}
          alt={item.title}
          className={`${thumbnailClasses} object-cover rounded`}
        />
  ...
  // Detailed variant - standard list with more details
  return (
    <div
