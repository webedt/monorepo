...
export default function WorkspaceLayout({ children }: WorkspaceLayoutProps) {
  ...
  // Track presence
  const { otherUsers, isConnected } = useWorkspacePresence({
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-base-100 border-b border-base-300 px-4 py-2">
        <div className="flex items-center justify-between">
          ...
          {/* Right: Presence indicators + GitHub link */}
          <div className="flex items-center gap-3">
              ...
              <div className="flex items-center">
                <div className="flex -space-x-2">
                  {otherUsers.slice(0, 3).map((user) => (
                    <div
                      key={user.userId}
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-medium ring-2 ring-base-100 ${getAvatarColor(user.displayName)}`}
                      title={`${user.displayName} (${user.page || 'unknown'})`}
                    >
                  ...
                  {otherUsers.length > 3 && (
                    <div
                      className="w-7 h-7 rounded-full flex items-center justify-center bg-base-300 text-base-content/70 text-xs font-medium ring-2 ring-base-100"
                ...
                <span className="ml-2 text-xs text-base-content/50 hidden md:inline">
            ...
            <div
              className={`w-2 h-2 rounded-full ${isConnected ? 'bg-success' : 'bg-base-content/30'}`}
