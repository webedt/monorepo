...
export default function Chat({ sessionId: sessionIdProp, isEmbedded = false }: ChatProps = {}) {
  ...
  const { disconnect: disconnectStream } = useEventSource(streamUrl, {
    ...
    onMessage: (event) => {
      ...
      // Capture title and branch from title_generation events
      ...
      if (data.type === 'title_generation' && data.status === 'success' && currentSessionId) {
        // Capture generated title (but don't overwrite user-edited title)
        if (data.title && !hasUserEditedTitleRef.current) {
          ...
          updateMutation.mutate({ id: currentSessionId, title: data.title });
        }
        // Capture generated branch name (will be replaced by actual branch with ID later)
        if (data.branch_name) {
      ...
      // Capture title from session_name events (fallback for title)
