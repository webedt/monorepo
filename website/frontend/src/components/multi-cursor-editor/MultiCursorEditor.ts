...
export class MultiCursorEditor extends Component<HTMLDivElement> {
  ...
  /**
   * Create the custom editor theme
   */
  private createCustomTheme(): Extension {
  ...
  /**
   * Create the multi-cursor keymap bindings
   */
  private createMultiCursorKeymap(): KeyBinding[] {
    return [
  ...
  /**
   * Create the base editor extensions
   */
  private createBaseExtensions(language: string): Extension[] {
    return [
      ...
      // Theme
      this.themeCompartment.of([this.createCustomTheme(), syntaxHighlighting(defaultHighlightStyle)]),
  ...
  private initializeEditor(): void {
    const state = EditorState.create({
      ...
      extensions: this.createBaseExtensions(this.options.language || 'text').flat(),
    });
  ...
  private createStateWithContent(content: string, language: string): EditorState {
    return EditorState.create({
      doc: content,
      extensions: this.createBaseExtensions(language).flat(),
    });
  }
