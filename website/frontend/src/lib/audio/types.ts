/**
 * Audio Source Types
 * Type definitions for the audio source system
 */
...
export type AudioSourceEvent =
  | { type: 'init' }
  | { type: 'play'; frequency: number }
  | { type: 'stop' }
  | { type: 'dispose' }
  | { type: 'waveformChange'; waveform: WaveformType }
  | { type: 'frequencyChange'; frequency: number }
  | { type: 'volumeChange'; volume: number };

/**
 * Note information for playback
 */
 ...
 * Audio source preset
 */
export interface AudioSourcePreset {
  /** Preset name */
  name: string;
  /** Waveform type */
  waveform: WaveformType;
...
/**
 * Standard audio source presets
 */
export const AUDIO_PRESETS: AudioSourcePreset[] = [
  {
    name: 'Default',
    waveform: 'sine',
  ...
  },
  {
    name: 'Bell',
    waveform: 'sine',
    envelope: { attack: 0.001, decay: 1.0, sustain: 0.0, release: 0.5 },
  },
];

/**
 * Standard frequencies for notes (A4 = 440Hz tuning)
 */
