...
// Use shared role hierarchy for validation
...
router.patch('/users/:id', requireAdmin, validateRequest(updateUserSchema), async (req, res) => {
  try {
    ...
    if (role !== undefined) {
      ...
      // Sync role with isAdmin - promote to admin or demote from admin
      if (isAdmin) {
        updateData.role = 'admin';
      } else {
        // When removing admin status, demote role to 'user'
        // This ensures role and isAdmin stay in sync
