...
// Use shared ROLE_HIERARCHY as single source of truth for valid roles
...
const createUserSchema = {
  body: z.object({
    ...
    role: z.enum(['user', 'editor', 'developer', 'admin'] as const).optional(),
  }),
};
...
router.patch('/users/:id', requireAdmin, validateRequest(updateUserSchema), async (req, res) => {
  try {
    ...
    // When role is explicitly set, sync isAdmin accordingly
    if (role !== undefined) {
      updateData.role = role;
      updateData.isAdmin = role === 'admin';
    } else if (isAdmin !== undefined) {
      updateData.isAdmin = isAdmin;
      // When isAdmin changes, sync role to prevent inconsistent state
      if (isAdmin) {
        updateData.role = 'admin';
      } else {
        // When demoting from admin, set role to 'user' to prevent
        // inconsistent state (role='admin' but isAdmin=false)
