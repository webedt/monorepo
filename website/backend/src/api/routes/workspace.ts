 ...
 */
router.put('/presence', collaborationRateLimiter, validateRequest(presenceUpdateSchema), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/events', collaborationRateLimiter, validateRequest(workspaceEventSchema), async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
